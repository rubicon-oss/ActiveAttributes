...
{
  [TestFixture]
  public class MethodAttributedAspectDeclarationProviderTest
  {
    [Test]
    public void GetDeclarations ()
    {
      ...
      aspectDeclarationHelperMock.Expect (x => x.GetAspectDeclarations (method)).Return (new[] { fakeDeclaration1, fakeDeclaration2 });
      aspectDeclarationHelperMock.Expect (x => x.GetAspectDeclarations (fakeMethod)).Return (new[] { fakeDeclaration3 });

      var provider = new MethodAttributedAspectDeclarationProvider (aspectDeclarationHelperMock, relatedMethodFinderMock);
